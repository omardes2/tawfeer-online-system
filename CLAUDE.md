...
## 🚫 نظام التوصيل (Optimus / أوبتيموس) — مُجمّد نهائيًا

نظام التوصيل مكتمل ومربوط فعليًا بشركة **Optimus**. **يُمنع منعًا نهائيًا** تعديله
أو إعادة بنائه أو تحسينه أو إعادة هيكلته (Refactor) — في أي مرحلة، ولأي سبب،
**حتى لو ظهرت طريقة أفضل للتنفيذ أو بدا المسار مخالفًا للمتوقَّع**. يُترك كما هو.
...
4. قبل أي عمل على السلة أو Checkout: يُفحص مسار التوصيل كاملًا وتُوثَّق أجزاؤه
   ...
   حولها فقط.
5. عند اكتشاف خلل في التوصيل: **يُذكر في التقرير فقط ولا يُصلَح ولا يُعاد البناء
   بسببه**، ويستمرّ باقي العمل. لا يجوز أن يوقف موضوعُ التوصيل إعادةَ تصميم
   المتجر — يُذكر أثره على التصميم إن وُجد، ويُكمَل ما أمكن.
6. عند تصميم السلة أو Checkout: يُغيَّر **الشكل حول** الوظائف القائمة فقط. تُحفظ
   كما هي: `name` و`id` و`value` والحقول المخفية و`data-*` وارتباطات Alpine
   ومحدّدات JavaScript والأحداث ومعاملات الـAPI. تُضاف أغلفة وأصناف تصميم
   حولها، ولا يُكسر العقد البرمجي.
