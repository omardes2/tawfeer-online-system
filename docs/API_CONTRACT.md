...
> **حالة التنفيذ:** ✅ مُنفَّذ ومُختبَر: الفروع والمستودعات والمواقع (2.1)؛ الكتالوج المرجعي (2.2)؛ **المنتجات ووسائطها (2.3)** — `/api/v1/products` (+ `products/{uuid}/images`) مع فلترة/بحث/ترتيب وحجب حقول التكلفة بـ `pricing.view_cost` (ADR-013، ADR-023). البقية مُصمَّمة وتُنفَّذ في مراحلها.
