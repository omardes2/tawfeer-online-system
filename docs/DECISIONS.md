...
## ADR-033 — إتمام الشراء (Checkout) [مُعتمد في Phase 3.2]
- **السياق:** الدفعة 3.2 من Phase 3 تحوّل السلة النشطة (3.1) إلى **طلب مبيعات** عبر الخدمات القائمة دون منطق أعمال جديد. لا مخطط جداول جديد (يُعاد استخدام `orders`/`payments`/المخزون). قرارات التصميم (موثّقة للمراجعة عند بوابة الاعتماد): **مصادَقة فقط** (اتساقًا مع سلة 3.1 ذاتية النطاق؛ سلة/إتمام الضيف مؤجّلان — ADR-031)، **نقطة نهاية ذرّية واحدة** `POST /store/checkout`، وعمق **طلب + بدء دفع** (يبقى تتبّع الشحن/الدورة في واجهات 2.6/2.7).
- **`CheckoutService` (تنسيقي بحت):** يقرأ سلة المستخدم، يرفض الفارغة، **يعيد التحقّق** من قابلية البيع والتوافر لكل بند (`CartService::assertPurchasable` — قد يتغيّر المخزون بعد الإضافة)، يشتقّ الفرع (`cart.branch_id ?? user.branch_id`) والمستودع (`branch.default_warehouse_id`)، ثم داخل **معاملة واحدة**: `OrderService::create` (لقطة عميل + بنود من السلة، `channel=web`) → `confirm` → `reserveStock` (**يحجز المخزون — معيار قبول المرحلة 3**) → `PaymentService::initiate` (COD يبقى pending حتى التحصيل عند التسليم — ADR-028) → تعليم السلة `converted`.
- **العميل واللقطة:** تُملأ لقطة العميل (اسم/هاتف/بريد/عنوان الشحن) من الطلب مباشرةً؛ إن كان للمستخدم عميل مرتبط (`customer_id`) يُضبط ويُطبّق منع العميل المحظور (BR-CUST-12 عبر `OrderService`). اللقطة ثابتة (ADR-030).
- **الأحداث المستقرّة (ADR-032):** `CheckoutStarted` (قبل المعاملة — نيّة/جاهزية هجر السلة) و`CheckoutCompleted` (**بعد نجاح المعاملة** — يُنشر خارج المعاملة، ADR-018). لا مستمع الآن — نقاط امتداد لسياق النمو، بلا كلفة.
- **العزل والوصول:** ذاتي النطاق بالمستخدم المُصادَق (لا صلاحية خاصة)؛ `CheckoutRequest` يتحقّق من لقطة الشحن وطريقة الدفع (`exists:payment_methods,code` + فحص `is_active` في الخدمة).
- **نقاط امتداد مؤجّلة موثّقة (ADR-032، بلا تنفيذ):** الكوبونات/العروض/محرّك التسعير، استرداد الولاء/المكافآت، طلبات الهدايا ورسائلها، Save For Later/Buy Again/Express/One-Click، اختيار عنوان محفوظ عبر API، تسعير الشحن الفعلي (يبقى `shipping_total=0` حتى مرحلته)، سلة/إتمام الضيف.
- **الاختبارات:** 9 اختبارات (منع الضيف، رفض السلة الفارغة، إتمام ناجح ينشئ طلبًا محجوزًا ودفعة، انعكاس الحجز على المخزون، تحويل السلة وبدء سلة جديدة، رفض طريقة دفع غير مُفعّلة/غير معروفة، رفض نقص المخزون بعد الإضافة، إطلاق الأحداث) → **308 ناجحة**.
