...
- **الفاتورة تصبح «مدفوعة» عند `in_accounting` (ADR-012g):** وصول المبلغ لمحاسبة المندوب هو إثبات أن **العميل دفع**، فتُحدَّث حالة الطلب: `amount_paid = total` و`payment_status = paid` عبر `OrderPaymentService::markCollected` — بنفس شرط قيمة البضاعة > 1، وضمن معاملة الانتقال، وidempotent. **بلا قيد محاسبي جديد ومقصودًا:** طلبات التوصيل تُرحَّل عند البيع على **«ذمم شركة التوصيل 1050»** لا على ذمم العميل، فالمبلغ مُثبَت كذمّة على الشركة منذ البيع، ويُقفَل لاحقًا بتسوية التوصيل (مدين الخزينة + رسوم التوصيل / دائن 1050). أي قيد هنا = ازدواج. مسار الإرجاع لا يمرّ بالحالة ⇒ لا تُعلَّم المرتجعات مدفوعة. تغيّر الحقول مسجَّل في Audit Log (الطلب `Auditable`).
- **تجاوز يدوي «تحويل إلى مدفوعة» — مدير النظام فقط:** حين لا تصل حالة المزوّد (تعطّل مزامنة/webhook) يستطيع الأدمن وحده (`OrderPolicy::markPaid` = `hasRole('admin')`) تعليم الفاتورة مدفوعة بنفس الأثر (حقول فقط، بلا قيد). **مشروط بوجود شحنة مُرسَلة فعلًا** لأن `OrderDeliveryDispatcher` يشتقّ «الدفع عند الاستلام» من `payment_status`، فتعليم طلب مدفوعًا قبل الإرسال يُخرج الشحنة بمبلغ تحصيل صفر. **المبيعات المباشرة (pos) مستثناة**: مديونيتها على العميل (1100) فتلزمها سند قبض بخزينة عبر زر «دفع».
