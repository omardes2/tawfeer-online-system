...
- **تجاوز يدوي «تحويل إلى مدفوعة» — مدير النظام فقط:** حين لا تصل حالة المزوّد (تعطّل مزامنة/webhook) يستطيع الأدمن وحده (`OrderPolicy::markPaid` = `hasRole('admin')`) تعليم الفاتورة مدفوعة **بنفس الأثر الكامل** (مدفوع + سند قبض في صندوق الأونلاين — ذرّيًا). **مشروط بوجود شحنة مُرسَلة فعلًا** لأن `OrderDeliveryDispatcher` يشتقّ «الدفع عند الاستلام» من `payment_status`، فتعليم طلب مدفوعًا قبل الإرسال يُخرج الشحنة بمبلغ تحصيل صفر. **المبيعات المباشرة (pos) مستثناة**: مديونيتها على العميل (1100) فتلزمها سند قبض بخزينة عبر زر «دفع».
