...
| **4.7** | تقارير العمليات (لوحات + أداء + إحصاءات + تصدير) | ✅ مكتملة |
...
### ✅ 4.7 — التقارير والتحليلات (المتطلّب 12) — مكتملة
> **مُنفَّذ (ADR-042):** وحدة `app/Modules/Reporting` — **طبقة قراءة فقط**، تجميعات SQL فوق جداول الأعمال القائمة (لا تكرار منطق، لا كتابة).
- **اللوحات (5):** تنفيذية، مبيعات، طلبات، توصيل، مالية. **التقارير (6):** أداء موظفي المبيعات، أداء المسوّقين، أداء المنتجات، إحصاءات العملاء، إحصاءات شركات التوصيل، إحصاءات المرتجعات — كلّها **مشتقّة من الدفاتر/الجداول** (العمولة من `commission_entries`، المال من `delivery_settlements`/`payments`، إلخ).
- **النطاق الزمني:** `DateRange` (يومي/أسبوعي/شهري/مخصّص). **بحث/فلترة سريعة** بالاسم على التقارير الجدولية.
- **التصدير:** Excel = CSV (UTF-8/BOM، تنزيل)؛ PDF = طباعة المتصفّح (CSS طباعة، عربية RTL صحيحة بلا اعتماديات). **رسوم** أشرطة CSS من الخادم (بلا JS)، متجاوبة للجوّال + RTL.
- **الصلاحيات:** `can:reports.view` (مدير/مشرف مبيعات/محاسب/مالية/عمليات توصيل). **10 اختبارات.** بذلك تكتمل المرحلة 4 (عدا 4.5 المُستبعَدة).
