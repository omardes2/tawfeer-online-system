<x-report.layout :title="__('كشف حساب').' — '.$treasury->name" :range="$range">
    ...
    <div class="bg-white shadow-sm rounded-lg p-5 overflow-x-auto">
        <table class="min-w-full text-sm text-right">
            <thead class="text-gray-500 border-b"><tr><th class="py-2 px-3">{{ __('التاريخ') }}</th><th class="py-2 px-3">{{ __('القيد') }}</th><th class="py-2 px-3">{{ __('رقم التتبّع') }}</th><th class="py-2 px-3">{{ __('الزبون') }}</th><th class="py-2 px-3">{{ __('البيان') }}</th><th class="py-2 px-3">{{ __('مدين') }}</th><th class="py-2 px-3">{{ __('دائن') }}</th><th class="py-2 px-3">{{ __('الرصيد') }}</th></tr></thead>
            <tbody class="divide-y">
                @php $run = $opening; @endphp
                <tr class="bg-gray-50"><td colspan="7" class="py-2 px-3 text-gray-500">{{ __('رصيد أول المدّة') }}</td><td class="py-2 px-3 font-bold">{{ number_format($run, 2) }}</td></tr>
                @forelse ($lines as $l)
                    ...
                    <tr>
                        ...
                        {{-- فاتورة شركة التوصيل تُكتب برقم التتبّع لا برقم الطلب. --}}
                        <td class="py-2 px-3 font-mono text-xs text-sky-700 select-all">{{ $trackings[$l->entry?->id] ?? '—' }}</td>
                        <td class="py-2 px-3 text-gray-700">{{ $parties[$l->entry?->id] ?? '—' }}</td>
                ...
                @empty
                    <tr><td colspan="8" class="py-6 text-center text-gray-400">{{ __('لا حركة في المدّة.') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
