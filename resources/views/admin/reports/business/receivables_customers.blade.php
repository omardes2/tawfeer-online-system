<x-app-layout :title="__('كشف حساب العملاء')">
    @php $sym = \App\Modules\Foundation\Services\Settings::get('store.currency_symbol', '₪'); @endphp
    <x-admin.header
        :title="__('كشف حساب العملاء')"
        :breadcrumbs="[__('الرئيسية') => route('admin.dashboard'), __('التقارير') => null, __('كشف حساب العملاء') => null]" />

    <p class="mb-4 text-sm text-gray-500">{{ __('المبلغ المستحق على كل عميل (رصيد حسابه في دفتر الأستاذ).') }}</p>
        ...
        <tbody>
            ...
            @empty
                <tr><td colspan="2" class="!p-0">
                    <x-admin.empty-state :title="__('لا يوجد عملاء')" :description="__('لم يُسجّل أي عميل بعد.')" />
                </td></tr>
            @endforelse
        </tbody>
        @if ($rows->isNotEmpty())
