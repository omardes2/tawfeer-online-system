<x-app-layout :title="$reportTitle">
    ...
    @include('admin.reports.business._basis', [
        'basisNote' => match (true) {
            // تقريرٌ عن أشخاصٍ بأعيانهم: إجماليّه أقلّ من مبيعات الفترة عمدًا،
            // ويُقال ذلك صراحةً كي لا يُقرأ الفرقُ خللًا.
            (bool) ($earnersOnly ?? false) => __('التقرير مقصور على الطلبات المرتبطة بـ:person — طلبات المتجر وما يُنشئه المدير خارجه، فمجموعه أقلّ من مبيعات الفترة. اضغط اسم :person لعرض تفصيل طلباته.', ['person' => $personLabel]),
            $rows->contains('unassigned', true) => __('صفّ «:unassigned» يضمّ طلبات المتجر الإلكتروني وما يُنشئه المدير، ووجودُه يجعل إجمالي هذه الصفحة مطابقًا لتقريري «المبيعات حسب الزبون» و«المبيعات حسب المنتج» لنفس الفترة. اضغط اسم :person لعرض تفصيل طلباته.', ['unassigned' => $unassignedLabel, 'person' => $personLabel]),
            default => __('التقرير مقصور على المحدَّدين في الفلتر، فمجموعه أقلّ من مبيعات الفترة. اضغط اسم :person لعرض تفصيل طلباته.', ['person' => $personLabel]),
        },
    ])
</x-app-layout>
