<x-report.layout :title="__('reports.marketers')" :range="$range" :searchable="true" :exportable="true">
    <div class="bg-white shadow-sm sm:rounded-lg p-4 overflow-x-auto">
        <table class="min-w-full text-sm text-right">
            <thead class="text-gray-500 border-b"><tr>
                ...
                <th class="py-2 px-3 font-medium">{{ __('reports.earnings') }}</th>
            </tr></thead>
            <tbody>
                @forelse ($rows as $r)
                    <tr class="border-b">
                        ...
                        <td class="py-2 px-3">{{ number_format((float) $r->earnings, 2) }}</td>
                    </tr>
                @empty
