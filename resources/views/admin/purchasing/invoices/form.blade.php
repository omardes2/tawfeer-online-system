<x-app-layout :title="$editing ? __('تعديل فاتورة شراء') : __('فاتورة شراء جديدة')">
    ...
    @if (! empty($stockShortages))
        <x-admin.alert tone="red" :title="__('لا يمكن حفظ التعديل على هذه الفاتورة')" class="mb-6">
            ...
            {{--
                لا إحالة إلى «عكس الفاتورة»: العكس يسحب البضاعة كما يسحبها
                التعديل، فيقف عند الفحص نفسه.
            --}}
            <p class="mt-2">
                {{ __('بِيعت الكمية أو نُقلت أو وُزِّعت على مقاسات أخرى.') }}
            </p>
            <p class="mt-1">
                {{ __('إن كان الصنف قد وُزِّع على مقاسات فالمخزون سليم ولا يحتاج تعديلًا — وزّع بند الفاتورة على مقاساته بدل تعديلها. وإلّا أعِد الكمية للمستودع بتسوية أولًا.') }}
            </p>
        </x-admin.alert>
    @endif
