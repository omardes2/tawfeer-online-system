        ...
        <div class="bg-white shadow-sm sm:rounded-lg p-6">
            ...
            <x-admin.header :title="__('أوامر الشراء')">
                @can('create', \App\Modules\Purchasing\Models\PurchaseOrder::class)
                    <a href="{{ route('admin.purchasing.orders.create') }}" class="inline-flex px-4 py-2 bg-emerald-600 text-white text-sm rounded-md hover:bg-emerald-700">{{ __('أمر شراء جديد') }}</a>
                @endcan
            </x-admin.header>
                    ...
                    <tbody class="divide-y">
                        @forelse ($orders as $o)
                            <tr>
                                ...
                                <td class="py-2 px-3"><a href="{{ route('admin.purchasing.orders.show', $o) }}" class="text-emerald-600 hover:underline">{{ __('عرض') }}</a></td>
                            </tr>
                        @empty
