        ...
        <div class="bg-white shadow-sm sm:rounded-lg p-6">
            ...
            <form method="POST" action="{{ $product->exists ? route('admin.products.update', $product) : route('admin.products.store') }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <x-admin.field :label="__('الاسم (عربي)')" name="name"><input type="text" name="name" value="{{ old('name', $product->name) }}" required class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" /></x-admin.field>
                    <x-admin.field :label="__('الاسم (إنجليزي)')" name="name_en"><input type="text" name="name_en" value="{{ old('name_en', $product->name_en) }}" class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" /></x-admin.field>
                    <x-admin.field :label="__('الرابط (slug) — اختياري')" name="slug"><input type="text" name="slug" value="{{ old('slug', $product->slug) }}" class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" /></x-admin.field>
                    <x-admin.field label="SKU" name="sku"><input type="text" name="sku" value="{{ old('sku', $product->sku) }}" required class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" /></x-admin.field>
                    <x-admin.field :label="__('الباركود')" name="barcode"><input type="text" name="barcode" value="{{ old('barcode', $product->barcode) }}" class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" /></x-admin.field>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <x-admin.field :label="__('العلامة التجارية')" name="brand_id">
                        <select name="brand_id" class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <x-admin.field :label="__('الوحدة')" name="unit_id">
                        <select name="unit_id" required class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                            <option value="">{{ __('— اختر —') }}</option>
                ...
                {{-- الأوصاف --}}
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <x-admin.field :label="__('وصف مختصر (عربي)')" name="short_description"><input type="text" name="short_description" value="{{ old('short_description', $product->short_description) }}" class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" /></x-admin.field>
                    <x-admin.field :label="__('وصف مختصر (إنجليزي)')" name="short_description_en"><input type="text" name="short_description_en" value="{{ old('short_description_en', $product->short_description_en) }}" class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" /></x-admin.field>
                    <x-admin.field :label="__('الوصف (عربي)')" name="description"><textarea name="description" rows="3" class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $product->description) }}</textarea></x-admin.field>
                    <x-admin.field :label="__('الوصف (إنجليزي)')" name="description_en"><textarea name="description_en" rows="3" class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">{{ old('description_en', $product->description_en) }}</textarea></x-admin.field>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
                    ...
                    <x-admin.field :label="__('الظهور')" name="visibility">
                        <select name="visibility" class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <x-admin.field :label="__('الترتيب')" name="sort_order"><input type="number" min="0" name="sort_order" value="{{ old('sort_order', $product->sort_order ?? 0) }}" class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" /></x-admin.field>
                    <label class="flex items-center gap-2 text-sm pb-2"><input type="hidden" name="is_featured" value="0" /><input type="checkbox" name="is_featured" value="1" @checked(old('is_featured', $product->is_featured)) class="rounded border-gray-300 text-indigo-600" />{{ __('مميّز') }}</label>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2 max-h-32 overflow-y-auto p-2 border rounded-md">
                            ...
                            @forelse ($tags as $tag)
                                <label class="inline-flex items-center gap-1 text-sm"><input type="checkbox" name="tag_ids[]" value="{{ $tag->id }}" @checked(in_array($tag->id, $selTags)) class="rounded border-gray-300 text-indigo-600" />{{ $tag->name }}</label>
                            @empty
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2 max-h-32 overflow-y-auto p-2 border rounded-md">
                            ...
                            @forelse ($attributes as $attr)
                                <label class="inline-flex items-center gap-1 text-sm"><input type="checkbox" name="attribute_ids[]" value="{{ $attr->id }}" @checked(in_array($attr->id, $selAttrs)) class="rounded border-gray-300 text-indigo-600" />{{ $attr->name }}</label>
                            @empty
                ...
                <div class="flex gap-2 pt-2"><button class="px-4 py-2 bg-indigo-600 text-white text-sm rounded-md hover:bg-indigo-700">{{ __('حفظ') }}</button><a href="{{ route('admin.products.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 text-sm rounded-md">{{ __('إلغاء') }}</a></div>
            </form>
        </div>
        ...
        @if ($product->exists)
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('admin.products.images.store', $product) }}" enctype="multipart/form-data" class="flex flex-wrap items-end gap-3">
                    ...
                    <label class="flex items-center gap-1 text-sm"><input type="checkbox" name="is_primary" value="1" class="rounded border-gray-300 text-indigo-600" />{{ __('أساسية') }}</label>
