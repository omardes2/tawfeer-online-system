    ...
    <div class="py-8 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white shadow-sm sm:rounded-lg p-6">
            ...
            <x-admin.header :title="__('سمات المنتجات')">
                @can('create', \App\Modules\Catalog\Models\ProductAttribute::class)<a href="{{ route('admin.attributes.create') }}" class="inline-flex px-4 py-2 bg-indigo-600 text-white text-sm rounded-md hover:bg-indigo-700">{{ __('سمة جديدة') }}</a>@endcan
            </x-admin.header>
                    ...
                    @forelse ($attributes as $attribute)
                        <tr>
                            ...
                            <td class="py-2 px-3"><div class="flex gap-2">
                                @can('update', $attribute)<a href="{{ route('admin.attributes.edit', $attribute) }}" class="text-indigo-600 hover:underline">{{ __('تعديل') }}</a>@endcan
