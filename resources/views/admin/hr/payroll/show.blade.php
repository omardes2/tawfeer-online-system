<x-app-layout :title="__('كشف :p', ['p' => $run->periodLabel()])">
    ...
    <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4 mb-5">
        ...
        <x-admin.stat-card :label="__('مخصّص نهاية الخدمة')" :value="$run->total_eos" money tone="amber"
                           :hint="__('لا يُصرف مع الراتب — يُصرف نهاية السنة')" />
    </div>
    ...
    <div class="admin-card admin-card-pad mb-5 text-sm text-gray-600 leading-7">
        ...
        <ul class="list-disc ps-5 space-y-1">
            ...
            <li>
                ...
                <a href="{{ route('admin.hr.eos.index') }}" class="text-emerald-700 hover:underline">{{ __('شاشة صرف نهاية الخدمة') }}</a>{{ __('، ولا يمرّ بالمصروف ثانيةً.') }}
            </li>
                ...
                <table class="admin-table">
                    <thead>
                        <tr>
                            ...
                            <th class="text-start">{{ __('نهاية الخدمة (مخصّص)') }}</th>
                            <th>{{ __('الصرف') }}</th>
                        </tr>
                    </thead>
                    <tbody>
