...
class PurchaseInvoiceService
{
    ...
    private function assertStockReturnable(PurchaseInvoice $invoice, Warehouse $warehouse): void
    {
        ...
        /*
            لا تُحِل على «اعكس الفاتورة»: العكس يسحب البضاعة كما يسحبها التعديل،
            فيقف عند هذا الفحص نفسه. وكانت الرسالة تُحيل إليه حين كان العكس
            يترك البضاعة — أي أنها كانت تدلّ على الطريق الذي يُفسد الدفتر.
        */
        throw ValidationException::withMessages(['items' => __(
            ...
            .'إن كان الصنف قد وُزِّع على مقاسات فالمخزون سليم ولا يحتاج تعديلًا — '
            .'وزّع بند الفاتورة على مقاساته بدل تعديلها. وإلّا أعِد الكمية للمستودع بتسوية أولًا.',
     ...
     *
     * **ويسحب البضاعة كما يعكس القيد.** كانت تعكس القيد وحده وتترك البضاعة في
     * المستودع: تختفي قيمة المشتريات وذمّة المورد من الدفاتر ويبقى الرصيد
     * الموجب — بضاعةٌ بلا تكلفة مقابلة، وذمّةٌ تقول إنك لا تدين لموردك وأنت
     * مدين. نصفُ عكسٍ أسوأ من لا عكس، لأنه يبدو تمامًا.
     *
     * والسحب يسبق عكس القيد: `reverseStock()` ترمي إن لم تعد البضاعة متاحة،
     * فتسقط المعاملة كلّها قبل أن يُمسّ الدفتر — تُمنع العملية بتمامها ولا
     * تُنفَّذ نصفًا.
     */
