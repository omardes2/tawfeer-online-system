...
class CommissionService
{
     ...
     *
     * ## لماذا لا يُمحى السند
     *
     * السند وثيقةٌ خرج بها مالٌ من الخزينة وله قيدٌ مُرحَّل في الدفتر. ومحوُه
     * يُنقص الخزينة رصيدًا بلا سبب ظاهر، ويترك قيدًا يتيمًا أو يمحو حركةً حدثت
     * فعلًا (BR-ACC-09). فالصواب **قيدٌ عاكس**: المال يعود في الدفاتر، ويبقى
     * الأصلُ والعكسُ مقروءين — «صُرف ثم أُلغي» لا «لم يُصرف قطّ».
     *
     * وبعد العكس يُخفى صفّ الأرشيف حذفًا ناعمًا، فيبقى مربوطًا بسنده لمن يسأل
     * عنه لاحقًا.
     *
     * ## وأثره على الرصيد — وهو المقصود
     *
     * `balance()` تعدّ المدفوع من السندات المُرحَّلة وحدها. فعكسُ السند يُخرجه
     * من المدفوع، و**يرتفع المتبقّي للمسوّق بمقدار الدفعة** — وهذا صحيح: ماله
     * عاد إلى الخزينة فصار له عند الشركة.
     *
     * ودفعةٌ سندُها معكوسٌ أو ملغًى أصلًا لا تُغيّر رقمًا بحذفها: هي مُستثناة
     * من الحساب قبله.
     *
     * ## وحدّان يُرفض عندهما الحذف
     *
     * - **دفعةٌ بلا سند**: تُعدّ «مصروفة» في الحساب (دفعةٌ قديمة من نظامٍ سابق)،
     *   فحذفُها يرفع المتبقّي بلا قيدٍ يقابله — مالٌ يُطالَب به مرّتين بلا أثر.
     * - **دفعةٌ ببنودٍ مربوطة**: بنودها في حالة `paid`، وإرجاعُها ليس انتقالًا
     *   مسموحًا في آلة الحالة. تُعالَج من السند بيدِ محاسب لا آليًّا.
     *
     * @return array{voucher_action: string, amount: float}
     */
    public function deletePayout(CommissionPayout $payout, ?User $actor = null, ?string $reason = null): array
    {
        $voucher = $payout->voucher()->first();

        if (! $voucher) {
            throw ValidationException::withMessages(['payout' => __(
                'دفعةٌ بلا سند تُحتسب مصروفةً في الرصيد، وحذفُها يرفع المتبقّي بلا قيدٍ يقابله. '
                .'اربطها بسندٍ أوّلًا أو عالجها محاسبيًّا.',
            )]);
        }

        if ($payout->entries()->exists()) {
            throw ValidationException::withMessages(['payout' => __(
                'هذه الدفعة مربوطةٌ ببنود عمولةٍ صارت «مدفوعة» — إرجاعُها ليس انتقالًا مسموحًا. '
                .'عالجها من صفحة السند.',
            )]);
        }

        $amount = $payout->settledAmount();

        return DB::transaction(function () use ($payout, $voucher, $reason, $amount) {
            $vouchers = app(VoucherService::class);

            if ($voucher->status === 'posted') {
                $vouchers->reverse($voucher, $reason
                    ?: __('عكس دفعة أرباح :number بحذفها من الأرشيف', ['number' => $voucher->number]));
                $action = 'reversed';
            } elseif (in_array($voucher->status, ['draft', 'approved'], true)) {
                // لم يُرحَّل بعد فلا قيد يُعكس — يُلغى كي لا يُرحَّل لاحقًا بالخطأ.
                $vouchers->cancel($voucher);
                $action = 'cancelled';
            } else {
                // معكوسٌ أو ملغًى أصلًا: ماله عاد، ولا شيء يُفعل بالسند.
                $action = 'untouched';
            }

            // العمود ٢٥٥ حرفًا: ملاحظةٌ أطول تُقطع في قاعدة البيانات أو تُسقط
            // الحفظ، فتضيع المعاملة كلّها من أجل نصٍّ زائد.
            $note = trim(($payout->notes ? $payout->notes.' — ' : '')
                .__('حُذف من الأرشيف').($reason ? ': '.$reason : ''));

            $payout->update(['notes' => mb_substr($note, 0, 255)]);

            $payout->delete();

            return ['voucher_action' => $action, 'amount' => $amount];
        });
    }
