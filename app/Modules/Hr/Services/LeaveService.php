 ...
 * راتب شهرها في الكشف — وذاك خصمٌ من مصروفٍ قائم لا التزامٌ يُقيَّد.
 *
 * ## والدوام الكامل وحده يستحقّ
 *
 * العقد والدوام الجزئيّ أجرٌ مقابل عمل: لا رصيد إجازةٍ يتراكم لهما. ويبقى
 * تسجيل الإجازة ممكنًا للسجلّ — وغيرُ المدفوعة تُخصَم من راتبها كالجميع —
 * لكنّ **الرصيد صفر**، فلا يُخترع استحقاقٌ لا يقوم على اتفاق.
 *
...
class LeaveService
{
    ...
    {
        // الدوام الكامل وحده يتراكم له رصيد: العقد والدوام الجزئيّ أجرٌ مقابل
        // عمل بلا إجازةٍ سنوية. وتسجيلُ إجازةٍ لهما يبقى ممكنًا للسجلّ — لكنّ
        // الرصيد لا يُخترع.
        if (! $profile->accruesBenefits()) {
            return 0.0;
        }
    ...
    /** أيام الإجازة غير المدفوعة الواقعة في شهرٍ بعينه — ما يُخصَم من كشفه. */
